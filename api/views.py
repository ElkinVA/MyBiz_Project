...
class ProductViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    list: Список товаров с фильтрацией и поиском
    retrieve: Детальная информация о товаре
    """
    ...
    def get_queryset(self):
        """Переопределяем для поддержки дополнительных фильтров"""
        queryset = super().get_queryset()
        
        # Фильтр по наличию (stock > 0)
...
class PromotionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    list: Список активных промо-акций
    retrieve: Детальная информация о промо-акции
    """
    permission_classes = [AllowAny]
    pagination_class = StandardResultsSetPagination
